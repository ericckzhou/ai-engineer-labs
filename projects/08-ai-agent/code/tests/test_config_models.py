...
_ENV_KEYS = [
    ...
    "AGENT_REPO_ROOT", "AGENT_MAX_STEPS", "AGENT_TOKEN_BUDGET",
]
...
def test_config_exposes_agent_knobs(clean_env):
    # The per-project Config carries the agent's budget knobs with the documented defaults.
    cfg = clean_env.load_config()
    assert cfg.repo_root == "."
    assert cfg.max_steps == 10
    assert cfg.token_budget == 20000
