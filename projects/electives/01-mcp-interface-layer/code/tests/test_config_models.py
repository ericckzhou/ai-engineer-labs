...
def test_config_exposes_mcp_knobs(clean_env):
    # The per-project Config carries the server identity + the trust-boundary bounds
    # (read by security.py) with the documented defaults.
    cfg = clean_env.load_config()
    assert cfg.server_name == "personal-memory"
    assert cfg.resource_scheme == "memory"
    assert cfg.default_k == 5
    assert cfg.max_k == 50
    assert cfg.max_text_chars == 4000
    assert cfg.allowed_kinds == ("episodic", "semantic", "procedural")
