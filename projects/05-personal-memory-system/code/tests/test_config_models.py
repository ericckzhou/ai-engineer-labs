...
def test_config_exposes_memory_knobs(clean_env):
    # The per-project Config carries the retrieval-scoring knobs with the documented defaults.
    cfg = clean_env.load_config()
    assert cfg.decay_rate == pytest.approx(0.995)
    assert cfg.top_k == 5
    assert cfg.weights == (1.0, 1.0, 1.0)
