...
`ollama/nomic-embed-text`, 768-dim) — never hardcoded here. Pin ONE model for both the memories and
the query; mixing models makes relevance (cosine similarity) meaningless (carries over from Project 02).
...
def embed_one(text: str) -> list[float]:
    """Embed a single text. Convenience for queries and single memories.
...
def main() -> None:
    ...
    v = embed_one("the user prefers dark mode")
